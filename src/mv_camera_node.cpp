...
class MVCameraNode : public rclcpp::Node {
    ...
    explicit MVCameraNode(const rclcpp::NodeOptions &options)
        ...
        capture_thread_ = std::thread{[this]() -> void {
            ...
            while (rclcpp::ok()) {
                ...
                if (flip_image_)
                    CameraFlipFrameBuffer(image_msg_.data.data(),
                                          &s_frame_info_, 3);
   ...
   private:
    void declareParameters() {
        ...
        param_desc.integer_range[0].to_value =
            t_capability_.sSaturationRange.iMax;
        int saturation;
        CameraGetSaturation(h_camera_, &saturation);
        saturation =
            this->declare_parameter("saturation", saturation, param_desc);
    ...
    rcl_interfaces::msg::SetParametersResult parametersCallback(
        ...
        for (const auto &param : parameters) {
            if (param.get_name() == "exposure_time") {
                ...
                if (status != CAMERA_STATUS_SUCCESS) {
                    result.successful = false;
                    result.reason = "Failed to set analog gain, status = " +
                                    std::to_string(status);
                }
                ...
                if (status != CAMERA_STATUS_SUCCESS) {
                    result.successful = false;
                    result.reason = "Failed to set RGB gain, status = " +
                                    std::to_string(status);
                }
                ...
                if (status != CAMERA_STATUS_SUCCESS) {
                    result.successful = false;
                    result.reason = "Failed to set Gamma, status = " +
                                    std::to_string(status);
                }
